...
export default function TournamentList({ onSelectTournament }: TournamentListProps) {
  ...
  const { trigger: deleteTournament, isMutating: isDeleting } = useSWRMutation("tournament", deleteById)
  ...
  const handleDelete = async (tournamentId: number) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Kon toernooi niet verwijderen.",
        variant: "destructive",
      })
    }
  }
      ...
      <div className="p-6">
        {tournaments.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-700 mb-2">Geen actieve toernooien</h3>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <span data-cy="tournament_round">{tournament.rondes} rondes</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        <Users className="h-4 w-4" />
                        <span data-cy="tournament_participation">{tournament.participations.length} deelnemers</span>
                      </div>
                    </div>
                  </div>
