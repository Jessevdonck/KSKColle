...
import { Users, Euro, Search } from 'lucide-react'
import { format } from 'date-fns'
import { nl } from 'date-fns/locale'

export interface LidgeldUser {
  user_id: number
  voornaam: string
  achternaam: string
  email: string
...
type LidgeldManagementProps = {
  /** Geen volledige pagina-layout (gebruik binnen ledenbeheer) */
  embedded?: boolean
  /** Na opslaan (bv. ledenlijst-cache verversen) */
  onLidgeldUpdated?: () => void
}

export default function LidgeldManagement({ embedded = false, onLidgeldUpdated }: LidgeldManagementProps) {
  ...
  const handleUpdateLidgeld = async (userId: number, data: Record<string, unknown>) => {
    try {
      ...
      console.error('Failed to update lidgeld status:', err)
    }
  }
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="sm:w-48">
              ...
              <select
                id="filter"
                value={filterStatus}
                onChange={(e) => setFilterStatus(e.target.value as 'all' | 'paid' | 'unpaid' | 'expired')}
...
function LidgeldEditModal({ user, onClose, onSave }: {
  ...
  const getAugust31st = () => {
    ...
    return new Date(yearForAugust31, 7, 31)
  }
