...
/** Publieke spelerslijst: actieve leden (lid/jeugd/bond) of spelers met clubrating > 0. */
...
const mapUsersToPublicUsersWithRating = async (
  ...
  return usersWithLastRatingUpdate.map(({ user, ratingDifference }) => {
    return {
      ...
      membership_valid: isUserMember(user),
      is_youth: user.is_youth === true,
    };
  });
};
...
/** Jeugdleden met clubrating (publiek, zelfde velden als publicUsers). */
export const getPublicYouthUsers = async (): Promise<PublicUser[]> => {
  try {
    const users = await prisma.user.findMany({
      where: { is_youth: true },
    });
