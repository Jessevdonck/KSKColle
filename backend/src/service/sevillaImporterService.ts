...
export class SevillaImporterService {
  ...
  private normalizeName(name: string): string {
    ...
    if (
      normalized.includes("Thomas Buys-Devill") &&
      !normalized.includes("Thomas Buys-Devillé")
    ) {
      normalized = "Thomas Buys-Devillé";
    } else if (
      normalized.includes("Buys-Devill") &&
      !normalized.includes("Buys-Devillé")
    ) {
      normalized = normalized.replace("Buys-Devill", "Buys-Devillé");
    } else if (
      normalized.includes("Devill") &&
      !normalized.includes("Devillé")
    ) {
    ...
    try {
      ...
      if (groups.length > 1) {
        ...
        for (const group of groups) {
          const tournamentId = await this.importGroup(
            group,
            sevillaData,
            tournamentNameToUse,
            group.Name,
  ...
  private async importGroup(
    group: SevillaGroup,
    _sevillaData: SevillaTournament,
    tournamentName: string,
    ...
    try {
      ...
      if (incremental) {
        ...
        if (tournament) {
          ...
          const isYouthTournament = tournamentName
            .toLowerCase()
            .includes("jeugd");
      ...
      if (historySection.length > 0) {
        ...
        const playerMap = await this.importPlayers(
          playersWithGames,
        ...
        await this.importRoundsAndGames(
          playersWithGames,
          tournament.tournament_id,
          playerMap,
          incremental,
      ...
      const playerMapFinal = await this.importPlayers(
        latestRanking.Player,
        tournament.tournament_id,
        incremental,
      );

      // Sync participations to the export: remove participations for players no longer in the Sevilla file.
      // Otherwise players who left the tournament (or were removed from the export) keep showing in the standings.
      const userIdsInExport = new Set(playerMapFinal.values());
      if (userIdsInExport.size > 0) {
        const deleted = await prisma.participation.deleteMany({
          where: {
            tournament_id: tournament.tournament_id,
            user_id: { notIn: Array.from(userIdsInExport) },
          },
        });
        if (deleted.count > 0) {
          console.log(
            `Removed ${deleted.count} stale participation(s) for tournament ${tournament.tournament_id} (players no longer in export).`,
          );
        }
      }
  ...
  private async importPlayers(
    ...
    for (const sevillaPlayer of players) {
      ...
      const allMatches = potentialMatches.filter(
        (u) =>
      ...
      if (user) {
        ...
        const existingMapping = Array.from(playerMap.entries()).find(
          ([_, mappedUserId]) => mappedUserId === userId,
        );
      ...
      if (!existingParticipation) {
        ...
        console.log(
          `Updated participation for ${user.voornaam} ${user.achternaam} in tournament ${tournamentId} with RtgDif=${sevillaPlayer.RtgDif}`,
        );
      }
  ...
  private async importRoundsAndGames(
    players: SevillaPlayer[],
    tournamentId: number,
    ...
    console.log(
      `=== importRoundsAndGames called with ${players.length} players (incremental: ${incremental}) ===`,
    );
    ...
    console.log(
      `📦 Preloaded ${makeupRounds.length} makeup rounds for syncing`,
    );

    // Collect makeup game updates to batch at the end
    const makeupGameUpdates: Array<{
      game_id: number;
    ...
    if (group?.RoundHist) {
      for (const roundHist of group.RoundHist) {
        if (roundHist.Game && Array.isArray(roundHist.Game)) {
          ...
          console.log(
            `📋 Mapped ${roundGames.size} games for round ${roundHist.ID} from RoundHist`,
          );
        }
      }
    }
            ...
            if (!roundDates.has(ranking.Round)) {
              ...
              if (dateParts.length === 3) {
                ...
                if (day && month && year) {
                  ...
                  console.log(
                    `✅ Set round ${ranking.Round} date: ${ranking.Date} -> ${date.toISOString()}`,
                  );
                }
              }
            }
              ...
              if (!roundDates.has(roundHist.ID)) {
                ...
                if (dateParts.length === 3) {
                  ...
                  if (day && month && year) {
                    ...
                    console.log(
                      `✅ Set round ${roundHist.ID} date from RoundHist: ${roundHist.Date} -> ${date.toISOString()}`,
                    );
                  }
                }
              }
              ...
              if (dateParts.length === 3) {
                ...
                if (day && month && year) {
                  ...
                  if (!roundDates.has(history.Round)) {
                    ...
                    console.log(
                      `Parsed round ${history.Round} date from history: ${history.Date} -> ${date.toISOString()}`,
                    );
                  }
                }
              }
    ...
    if (roundDates.size > 0) {
      ...
      tournamentStartDate =
        roundDates.get(earliestRound) || new Date("2024-09-15");
      console.log(
        `Using tournament start date from Sevilla data: ${tournamentStartDate.toISOString()}`,
      );
    }
    ...
    const calculateFallbackDate = (roundNumber: number): Date => {
      ...
      if (roundDates.size > 0) {
        ...
        fallbackDate.setDate(
          fallbackDate.getDate() + roundsDifference * daysBetweenRounds,
        );
        console.log(
          `📅 Calculated fallback date for round ${roundNumber}: ${fallbackDate.toISOString()}`,
        );
        return fallbackDate;
      }
      ...
      console.log(
        `📅 Ultimate fallback date for round ${roundNumber}: ${fallbackDate.toISOString()}`,
      );
      return fallbackDate;
    };
    ...
    players.forEach((player) => {
      if (player.Game && Array.isArray(player.Game)) {
        ...
        console.log(
          `Player ${player.Name} has no games or games is not an array`,
        );
      }
    });
    ...
    console.log(
      `Found ${sortedRounds.length} unique rounds: ${sortedRounds.join(", ")}`,
    );
    ...
    console.log(
      `Existing rounds in tournament: ${existingRounds.map((r) => `${r.ronde_nummer}(${r.type || "REGULAR"})`).join(", ")}`,
    );
    ...
    for (const roundNumber of sortedRounds) {
      ...
      if (incremental) {
        ...
        const existingRound =
          sevillaRoundIndex >= 0 && sevillaRoundIndex < allSevillaRounds.length
            ? allSevillaRounds[sevillaRoundIndex]
            : null;

        if (existingRound) {
          console.log(
            `Found existing Sevilla round ${roundNumber} with ID ${existingRound.round_id} and ronde_nummer ${existingRound.ronde_nummer}`,
          );
          round = existingRound;

          // Check if round date needs to be updated
          const existingDate = existingRound.ronde_datum
            ? new Date(existingRound.ronde_datum)
            : null;
          ...
          if (needsDateUpdate || needsStartuurUpdate) {
            ...
            if (needsDateUpdate) {
              ...
              console.log(
                `Updating round ${roundNumber} date from ${existingDate?.toISOString()} to ${roundDate.toISOString()}`,
              );
            }
            ...
            if (needsStartuurUpdate) {
              ...
              console.log(
                `Setting round ${roundNumber} startuur to ${defaultStartuur}`,
              );
            }
            ...
            if (needsDateUpdate)
              console.log(`Updated round ${roundNumber} date`);
            if (needsStartuurUpdate)
              console.log(`Updated round ${roundNumber} startuur`);
          } else {
            console.log(
              `Round ${roundNumber} date and startuur are up to date`,
            );
          }
          ...
          const correctRoundNumber = await this.determineCorrectRoundNumber(
            tournamentId,
            roundNumber,
            existingRounds,
          );
          ...
          console.log(
            `Created new round ${roundNumber} with correct number ${correctRoundNumber} and ID ${round.round_id}`,
          );
        }
      } else {
        ...
        console.log(
          `Created round ${roundNumber} with correct number ${correctRoundNumber} and ID ${round.round_id}`,
        );
      }

      // Import games for this round
      await this.importGamesForRound(
        players,
        round.round_id,
        roundNumber,
        playerMap,
      ...
      await this.importAbsentPlayers(
        players,
        round.round_id,
        roundNumber,
        playerMap,
   ...
   */
  private async determineCorrectRoundNumber(
    _tournamentId: number,
    sevillaRoundNumber: number,
    existingRounds: any[],
  ): Promise<number> {
    ...
    const existingRoundWithNumber = existingRounds.find(
      (r) => r.ronde_nummer === sevillaRoundNumber,
    );
    ...
    if (existingRoundWithNumber) {
      ...
      const maxRegularRound = Math.max(
        ...existingRounds
          .filter((r) => r.type === "REGULAR")
          .map((r) => r.ronde_nummer),
      );
  ...
  private async importGamesForRound(
    players: SevillaPlayer[],
    roundId: number,
    roundNumber: number,
    playerMap: Map<number, number>,
    ...
    roundHistGames?: Map<number, Map<string, number>>,
  ) {
    ...
    for (const player of players) {
      if (!player.Game || !Array.isArray(player.Game)) {
        console.log(
          `Player ${player.Name} has no Game array or Game is not an array`,
        );
        continue;
      }
      ...
      if (!playerUserId || !opponentUserId) {
        console.warn(
          `Could not find user IDs for game: ${player.ID} vs ${playerGame.Opponent}`,
        );
        continue;
      }
      ...
      if (roundHistGames && roundHistGames.has(roundNumber)) {
        ...
        const whiteSevillaPlayer = players.find(
          (p) => p.Name === playerGame.White,
        );
        const blackSevillaPlayer = players.find(
          (p) => p.Name === playerGame.Black,
        );
        ...
        if (whiteSevillaPlayer && blackSevillaPlayer) {
          ...
          if (roundHistPosition) {
            ...
            console.log(
              `🎯 Using RoundHist board position ${boardPosition} for game ${sevillaGameKey} in round ${roundNumber}`,
            );
          } else {
            console.log(
              `⚠️ No RoundHist position found for game ${sevillaGameKey} in round ${roundNumber}, using fallback ${boardPosition}`,
            );
          }
        } else {
          console.log(
            `⚠️ Could not find Sevilla players for game ${playerGame.White} vs ${playerGame.Black} in round ${roundNumber}`,
          );
        }
      }
      ...
      const whiteSevillaPlayer = players.find(
        (p) => p.Name === whitePlayerName,
      );
      const blackSevillaPlayer = players.find(
        (p) => p.Name === blackPlayerName,
      );

      // Determine who is white and who is black in our database
      const whitePlayerId = whiteSevillaPlayer
        ? playerMap.get(whiteSevillaPlayer.ID) || null
        : null;
      const blackPlayerId = blackSevillaPlayer
        ? playerMap.get(blackSevillaPlayer.ID) || null
        : null;
      ...
      if (existingGame) {
        ...
        await prisma.game.update({
          ...
          data: {
            ...
            ...(hasRealResult
              ...
              : existingGame.uitgestelde_datum
                ? { uitgestelde_datum: existingGame.uitgestelde_datum }
                : {}),
          },
        });
        ...
        if (hasRealResult && makeupRounds.length > 0) {
          this.collectMakeupGameUpdate(
            existingGame.speler1_id,
            existingGame.speler2_id,
            result,
            winnaarId,
            makeupRounds,
            makeupGameUpdates,
    ...
    console.log(
      `Round ${roundNumber}: Found ${gamesFound} games, created ${gamesInRound.size} unique games`,
    );
  }
   ...
   */
  private async importAbsentPlayers(
    players: SevillaPlayer[],
    roundId: number,
    roundNumber: number,
    playerMap: Map<number, number>,
    ...
    for (const player of players) {
      ...
      const hasGameInRound =
        player.Game && player.Game.some((g) => g.Round === roundNumber);

      if (!hasGameInRound) {
        const playerUserId = playerMap.get(player.ID);
        if (!playerUserId) {
          console.warn(
            `Could not find user ID for absent player: ${player.ID}`,
          );
          continue;
        }
        ...
        const existingGame = await prisma.game.findFirst({
          where: {
            round_id: roundId,
            OR: [{ speler1_id: playerUserId }, { speler2_id: playerUserId }],
          },
        });

        if (existingGame) {
          console.log(
            `Player ${player.Name} already has a game in round ${roundNumber}, skipping`,
          );
          continue;
        }
        ...
        const hasAbsEntry =
          player.Abs &&
          player.Abs.some((abs: any) => abs.Round === roundNumber);

        if (hasAbsEntry && player.Abs) {
          const absEntry = player.Abs.find(
            (abs: any) => abs.Round === roundNumber,
          );
          const absScore = absEntry?.Score || 0;
          const reason = absEntry?.Reason || "";

          console.log(
            `Creating absent game for player: ${player.Name} (Reason: ${reason}, Score: ${absScore})`,
          );
          ...
          console.log(
            `Created absent game for player: ${player.Name} (ID: ${player.ID}) in round ${roundNumber} - ${reason} - ${result}`,
          );
        }
      }
    }
  ...
  async validateSevillaData(data: any): Promise<boolean> {
    try {
      ...
      if (
        !mainGroup ||
        !mainGroup.Ranking ||
        !Array.isArray(mainGroup.Ranking)
      ) {
        return false;
      }
      ...
      if (
        !latestRanking ||
        !latestRanking.Player ||
        !Array.isArray(latestRanking.Player)
      ) {
        return false;
      }
  ...
  private collectMakeupGameUpdate(
    ...
    for (const makeupRound of makeupRounds) {
      const makeupGame = makeupRound.games.find(
        (g: any) =>
      ...
      if (
        makeupGame &&
        (makeupGame.result !== result || makeupGame.winnaar_id !== winnaarId)
      ) {
